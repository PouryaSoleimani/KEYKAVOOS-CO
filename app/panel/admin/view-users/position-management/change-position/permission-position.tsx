...
function PermissionPosition() {
    ...
    //* ATTACH PERMISSION TO POSITION
    const onSubmitGreen: SubmitHandler<Inputs> = (data) => {
        console.log("GREEN", data)
        const position_id = data.position
        const permission_id = data.permission
        app.get(`/position/attach_permision/${position_id}/${permission_id}`)
            .then(response => {
                console.log(response.data);
                toast.success("جایگاه با موفقیت به دسترسی داده شد", { position: "top-right", autoClose: 2000, hideProgressBar: true, style: { fontSize: "14px" }, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light", transition: Bounce, rtl: true, });
                router.refresh()
            })
            .catch((error: any) => {
    ...
    //! DETACH PERMISSION FROM POSITION
    const onSubmitRed: SubmitHandler<Inputs> = (data) => {
        console.log(data, "RED")
        const permission_id = data.permission
        const position_id = data.position
        app.get(`/position/detach_permision/${position_id}/${permission_id}`)
            .then(response => {
                console.log(response.data);
                toast.success("جایگاه با موفقیت از دسترسی گرفته شد", { position: "top-right", autoClose: 1500, hideProgressBar: true, style: { fontSize: "14px" }, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light", transition: Bounce, rtl: true, });
                router.refresh()
            })
            .catch(error => {
        ...
        <div className="bg-white shadow mx-auto rounded-lg w-full p-[3%] space-y-3">
            <form className="grid grid-cols-1 gap-8">
                ...
                <div className="flex items-center justify-end space-x-6 w-full">
                    <button className="btn bg-green-800 text-white  mx-4 hover:bg-green-600 duration-300" onClick={handleSubmit(onSubmitGreen)} >دادن جایگاه به دسترسی</button>
...
export default PermissionPosition;
