...
function PermissionRole() {
    ...
    function GETALLPOSITIONS() { app.get('/positions').then(response => { setAllPositions(response.data.data); console.log("POSITIONS ==>", response.data) }) }
    useEffect(() => { GETALLPERMISSIONS(); GETALLROLES(); GETALLPOSITIONS() }, [])
    ...
    const onSubmitGreen: SubmitHandler<Inputs> = (data) => {
        ...
        app.get(`/role/attach_permission/${Number(role_id)}/${Number(permission_id)}`)
            .then(response => {
                console.log(response.data);
                toast.success("نقش با موفقیت به دسترسی داده شد", { position: "top-right", autoClose: 1500, hideProgressBar: true, style: { fontSize: "14px" }, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light", transition: Bounce, rtl: true, });
            })
    ...
    const onSubmitRed: SubmitHandler<Inputs> = (data) => {
        ...
        const permission_id = data.permission
        const role_id = data.role
        app.get(`/role/detach_permission/${role_id}/${Number(permission_id)}`)
            .then(response => {
                console.log(response.data);
                toast.success("نقش با موفقیت از دسترسی گرفته شد", { position: "top-right", autoClose: 2000, hideProgressBar: true, style: { fontSize: "14px" }, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light", transition: Bounce, rtl: true, });
            })
            .then(data => router.refresh())
            .catch((error: any) => {
                console.log(error.response);
        ...
        <div className="bg-white shadow mx-auto rounded-lg w-full p-[3%] space-y-3">
            <form className="grid grid-cols-1 gap-8">
                ...
                <div className="flex items-center justify-end space-x-6 w-full">
                    <button className="btn bg-green-800 text-white  mx-4 hover:bg-green-600 duration-300" onClick={handleSubmit(onSubmitGreen)} >دادن نقش به دسترسی</button>
                    <button className="btn bg-red-800 text-white mx-4 hover:bg-red-600 duration-300" onClick={handleSubmit(onSubmitRed)}  >گرفتن نقش از دسترسی</button>
                </div>
            </form>
        </div>
    );
