...
function PanelFields({ label, onChange, value, disable, name, readonly, flexDirection }: PanelFieldsProps) {
  ...
  return (
    <div className={`flex ${flexDirection ? `${flexDirection} items-center` : "flex-col"}  gap-1`}>
      ...
      <input className={`${disable ? "bg-[#EAEFF6] text-slate-500/60 px-3" : "bg-[#EAEFF6] px-2"} translate-x-1 px-2 text-zinc-800 text-lg font-extralight tracking-tighter outline-none border-none py-2 rounded-[4px] mx-2`}
        onChange={onChange}
        value={value}
        disabled={disable}
        name={name}
