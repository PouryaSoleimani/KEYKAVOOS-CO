...
function SubmitOrderDropdown({ dropDownTitle, dropdownItems, value, onChange, name, }: SubmitOrderDropdownProps) {
  ...
  return (
    <div className={`flex ${dropDownTitle ? "flex-col gap-1" : "flex-row"} relative`} >
      <p>{dropDownTitle}</p>
      <select name={name ? name : dropDownTitle} id={value} className={`bg-[#EAEFF6] h-full rounded-[4px] text-zinc-700 font-YekanBakh tracking-tighter text-md font-extralight px-3 py-2 ${!dropDownTitle && "w-full font-YekanBakh"}`} value={`${value}`} onChange={onChange}>
