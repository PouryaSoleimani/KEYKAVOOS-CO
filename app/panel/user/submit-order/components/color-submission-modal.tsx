...
type ColorSubmissionModalProps = {
  ...
  modalInputValue: { title: string; color: string; };
  setModalInputValue: React.Dispatch<
    React.SetStateAction<{ title: string; color: string; }>>;
};

function ColorSubmissionModal({ showModal, setShowModal, data, setData, modalInputValue, setModalInputValue, }: ColorSubmissionModalProps) {
        ...
        <div className="relative p-8 w-full max-w-2xl max-h-full">
          <div className="bg-white rounded-[25px] shadow border">
            ...
            <div className="md:px-5 flex flex-row-reverse gap-5">
              {data.map((item) => (
                <p key={item.color} className={`bg-[#EAEFF6] rounded-[4px] px-2 ${item.color ? "inline-block" : "hidden"}`} >
                  {item.color}
                </p>
              ))}
            </div>
